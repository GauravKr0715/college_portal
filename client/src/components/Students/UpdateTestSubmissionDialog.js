...
import {
  editTestSubmissionWithoutAttach,
  editTestSubmissionWithAttach,
} from "../../services/student";
import { createTheme, ThemeProvider, styled } from "@mui/material/styles";
import { createMuiTheme } from "@material-ui/core";

function UpdateTestSubmissionDialog(props) {
  useEffect(() => { }, []);
    ...
    try {
      ...
      if (((old_files.length <= 0) && (response === '' || response === null) && (new_files === '' || new_files === null || new_files.length <= 0))) {
        ...
        if (document.getElementById("attachments").files.length) {
          ...
          const { data } = await editTestSubmissionWithAttach(
            formData,
            props.submission_id
          );
          setNewFiles(null);
          ...
          console.log(details);
          const { data } = await editTestSubmissionWithoutAttach(
            details,
            props.submission_id
          );
          console.log(data);
...
export default UpdateTestSubmissionDialog;
