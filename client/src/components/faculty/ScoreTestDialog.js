...
import "./score.css";
import { scoreTestSubmission } from "../../services/faculty";
import { createTheme, ThemeProvider, styled } from "@mui/material/styles";
import { createMuiTheme } from "@material-ui/core";

function ScoreTestDialog(props) {
  useEffect(() => { }, []);
  ...
  const saveScore = async () => {
    try {
      ...
      if (marks_scored === null || marks_scored === "") {
        ...
        console.log(details);
        const { data } = await scoreTestSubmission(
          details,
          props.submission_id
        );
        console.log(data);
          ...
          >
            ...
            >
              ...
              <TextField
                ...
                label="Marks Scored"
                type="number"
                inputProps={{ inputMode: 'numeric', pattern: '[0-9]*', min: 0, max: `${props.test.total_marks}` }}
                value={marks_scored}
                onChangeCapture={(e) => {
              ...
              />
              <div className="total-marks-container">{`/ ${props.test.total_marks}`}</div>
            </Box>
          </Box>
        </DialogContent>
        <DialogActions
...
export default ScoreTestDialog;
